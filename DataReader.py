...
def fashion_mnist_reader(args, path):
	...
	if args.is_train == "True":
		trX = tf.convert_to_tensor(trainX, dtype=tf.float32) / 255.
		trY = tf.convert_to_tensor(trainY, dtype=tf.float32)
	...
	input_queue = tf.train.slice_input_producer([trX, trY],shuffle=True)
